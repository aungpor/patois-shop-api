...
(SELECT count(*) FROM patois_merchant_shop_post_review_like where post_review_id = wpr.post_review_id) as post_review_like_count,
(select (
       ( select count(*) from patois_merchant_shop_post_review_comment WITH (NOLOCK)  where post_review_id = wpr.post_review_id and active != 0)+
       ( select count(*) from patois_merchant_shop_post_comment_reply WITH (NOLOCK)  where comment_id in ( select comment_id from patois_merchant_shop_post_review_comment WITH (NOLOCK)  where post_review_id = wpr.post_review_id and active != 0) )
)) as comment_count,
'merchant_shop' as types
FROM dbo.patois_merchant_shop_post_review wpr WITH (NOLOCK) 
left join users u WITH (NOLOCK) on u.id = wpr.user_id 
left join patois_merchant_shop_review_score wrs WITH (NOLOCK) on wrs.user_id = wpr.user_id  and wrs.shop_id = wpr.shop_id and wrs.post_review_id = wpr.post_review_id
left join patois_images pi2 WITH (NOLOCK) on pi2.images_id = wpr.images_id 
left join patois_merchant_shop_post_review_like wprl WITH (NOLOCK) on wprl.post_review_id = wpr.post_review_id and wprl.shop_id = wpr.shop_id and wprl.user_id = @user_id
